...
public class CameraUtil {
     ...
     * @return
     */
    public Size getPropSizeForHeight(List<Size> list, int maxHeight) {
     ...
     * @return
     */
    public Size getPropSizeForWidth(List<Size> list, int maxWidth) {
        Collections.sort(list, new CameraAscendSizeComparatorForWidth());

        int i = 0;
        for (Size s : list) {
            if ((s.width <= maxWidth)) {
                LogUtils.i("s.width===" + s.width);
                break;
            }
            i++;
        }
        if (i == list.size()) {
            i = list.size();//如果没找到，就选最小的size
        }
        return list.get(i);
    }

    //降序 按照宽度
    public class CameraAscendSizeComparatorForWidth implements Comparator<Size> {
        public int compare(Size lhs, Size rhs) {
     ...
     */
    public Camera.Size getCloselyPreSize(int surfaceWidth, int surfaceHeight,
                                            List<Size> preSizeList, boolean isPortrait) {
    ...
    public void printSupportPreviewSize(Camera.Parameters params) {
        ...
        for (int i = 0; i < previewSizes.size(); i++) {
            ...
            LogUtils.i("printSupportPreviewSize " + size.width + "*" + size.height);
        }
    ...
    public void printSupportPictureSize(Camera.Parameters params) {
        ...
        for (int i = 0; i < pictureSizes.size(); i++) {
            ...
            LogUtils.i("printSupportPictureSize " + size.width + "*" + size.height);
        }
    }
